...
class PromotionsForm extends React.Component {
	...
	ApplicationsBonus = (ID) => {
		PiwikEvent('Promo Application', 'Submit', `Apply_(${ID})_EUROPage`)
	...
	render() {
		const { title, contentId, status, manualRemark } = this.props.Detail;
		const { FirstName, Contacts } = this.props.memberInfo;
			...
			<View style={{ flex: 1, backgroundColor: '#fff', padding: 15, }}>
				...
				>
					...
					<View>
						...
						<View style={styles.viewInput}>
							<Text style={{ color: '#202939' }}>{ContactsEmail.Contact && `*******${ContactsEmail.Contact.split('@')[0].slice(-3)}@${ContactsEmail.Contact.split('@')[1]}`}</Text>
						</View>

						<View style={[styles.viewInput,{backgroundColor: '#F7F7FC', marginTop: 6}]}>
							<Touch style={{ color: '#666666', fontSize: 12}} onPress={() => {  LiveChatOpenGlobe() }}>
								<Text style={{ color: '#666666', fontSize: 12}}>如果您想更新电子邮箱，请联系我们的<Text style={{ color: '#00a6ff', textDecorationLine: 'underline',fontSize: 12 }}>在线客服</Text>
							</Text>
							</Touch>
						</View>
					</View>
					<View>
						...
						<Touch style={[styles.viewInput,{backgroundColor: '#F7F7FC', marginTop: 6}]} onPress={() => {  LiveChatOpenGlobe() }}>
							<Text style={{ color: '#666666', fontSize: 12}}>如果您想更新联系电话，请联系我们的<Text style={{ color: '#00a6ff', textDecorationLine: 'underline',fontSize: 12 }}>在线客服</Text>
							</Text>
						</Touch>
					</View>
					<View>
					...
					>
						<View style={styles.modalMaster}>
							...
							<Image style={{ width: 60, height: 60 }} source={require('../../../../images/icon-done.png')} />
