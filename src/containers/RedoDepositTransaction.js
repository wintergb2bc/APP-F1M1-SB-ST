...
class RedoDepositTransaction extends React.Component {
    ...
    getResubmitOnlineDepositDetails(item) {
        ...
        fetchRequest(window.ApiPort.GetResubmitOnlineDepositDetails + 'resubmitDepositID=' + this.state.recordItem.TransactionId + '&', 'GET').then(res => {
            Toast.hide()
    ...
    CreateResubmitOnlineDeposit(res) {
        Toast.loading('加载中,请稍候...', 2000)
        fetchRequest(window.ApiPort.CreateResubmitOnlineDeposit + `resubmitDepositID=${this.state.recordItem.TransactionId}&returnUrl=` + SBTDomain + '&', 'POST').then(res => {
            Toast.hide()
