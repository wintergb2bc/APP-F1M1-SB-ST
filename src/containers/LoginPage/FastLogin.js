...
class FastLogin extends React.Component {
	...
	componentWillMount() {
		...
		if(LoginTouchNum > 2 || (((DeviceInfoIos && LoginTouchNum > 1) || (!DeviceInfoIos && LoginTouchNum > 0)) && Platform.OS == "ios")) {
			...
			let message = DeviceInfoIos ? "脸部辨识失败4次，请使用一般登入或是联系客服。" : "指纹辨识失败3次，请使用一般登入或是 联系客服。";
	...
	onStart() {
		...
		this.setState({ status: "normal", message: '请输入您的图形密码', scrollEnabled: false });
	...
	getBtnPos = (e) => {
		NativeModules.UIManager.measure(e.target, (x, y, width, height, px, py) => {
			...
			console.log(window.LoginPatternHeight, 'pypypypypypypy1111',py, this.state.winHeight)
		});
	}
			...
			<View style={{ flex: 1, backgroundColor: '#fff' }}>
					...
					<View>
						...
						<View style={styles.loginView}>
							<View style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
								...
								/>
								<Text style={{ textAlign: 'center', lineHeight: 50 }}>{DeviceInfoIos ? '脸部辨识快速登录' : '使用指纹辨识'}</Text>
								<Touch style={{ backgroundColor: '#00A6FF', borderRadius: 8, width: width - 40, marginTop: 30 }} onPress={() => { Actions.pop() }}>
									<Text style={{ color: '#fff', textAlign: 'center', lineHeight: 42 }}>一般登入</Text>
								</Touch>
							</View>
						</View>
					</View>
...

				{
					...
					onScroll={(e) => {
						...
						window.LoginPatternHeight = window.LoginPatternHeight + offsetY - ScrollNum
						console.log(window.LoginPatternHeight, 'offsetY.offsetY',offsetY)
					...
					style={{flex: 1}}>
						...
						<Touch style={{ backgroundColor: '#00A6FF', borderRadius: 8, width: width - 40, marginTop: 30, marginLeft: 20, marginBottom: 50 }} onPress={() => { Actions.pop() }}>
							<Text style={{ color: '#fff', textAlign: 'center', lineHeight: 42 }}>一般登入</Text>
						</Touch>
					</ScrollView>
				}
