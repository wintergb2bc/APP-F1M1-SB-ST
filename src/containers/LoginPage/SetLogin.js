...
class SetLogin extends React.Component {
	...
	setLogin(key) {
		...
		if(this.state.loginType == key) { return }
		...
		if(key == 1) {
			Actions.LoginTouch({ username, fastChange: true, changeBack: () => { this.setState({loginType: 1}) } })
		}

		if(key == 2 || key == 3) {
			Actions.LoginPattern({ username, fastChange: true, forgotPass: key == 3, changeBack: () => { this.setState({loginType: 2}) } })
		}
		...
		return (
			<View style={{ flex: 1, backgroundColor: '#EFEFF4', padding: 20 }}>
				<View style={styles.setLogin}>
					<Touch style={styles.setTouch} onPress={() => { this.setLogin(1) }}>
						<Text style={{ color: '#666' }}>使用指纹辨识登入</Text>
						<View style={[loginType == 1 ? styles.loginTouch : styles.setDefual]} />
					</Touch>
					<Touch style={styles.setTouch} onPress={() => { this.setLogin(2) }}>
						<Text style={{ color: '#666' }}>使用图形密码</Text>
						<View style={[loginType == 2 ? styles.loginTouch : styles.setDefual]} />
					</Touch>
					{
						loginType == 2 &&
						<Touch style={[styles.setTouch, { paddingLeft: 30, paddingRight: 10 }]} onPress={() => { this.setLogin(3) }}>
					...
					<Touch style={styles.setTouch} onPress={() => { this.setLogin(0) }}>
						<Text style={{ color: '#666' }}>关闭所有快速登入功能</Text>
						<View style={[loginType == 0 ? styles.loginTouch : styles.setDefual]} />
					</Touch>
				</View>
			</View>
		);
