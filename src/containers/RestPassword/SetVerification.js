...
class SetVerification extends React.Component {
	...
	getVerifyTimes() {
		...
		fetchRequest(ApiPort.VerificationAttempt + `?ServiceAction=Revalidate&channelType=${types}&`, "GET")
		.then(res => {
			Toast.hide();
	...
	verifyEmailTAC() {
		...
		Toast.loading("验证中...", 100);
		fetchRequest(ApiPort.EmailTAC, "POST", data)
			.then(res => {
				Toast.hide();
				if (res) {
					if (!res.isSuccess) {
						...
						Toast.fail(res.result.errorMessage, 2);
						this.clearCode();
					} else {
						Toasts.success("验证成功", 2);
						Actions.pop()
