	...
	getDetail = (type, data) => {
		...
		fetchRequest(fetchurl, "GET")
			.then((res) => {
				Toast.hide();
				if (res) {
					let detail = type == "Announcement" ? res.AnnouncementResponse : res.PersonalMessage;
	...
	getNextPage = (type) => {
		...
		switch (type) {
			...
			case "Transfer":
				...
				break;
			case "Announcement":
				fetchurl = `${ApiPort.GetAnnouncements}MessageTypeOptionID=${selItemAnnouncement.id
