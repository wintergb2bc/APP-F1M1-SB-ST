...
class DepositPage extends React.Component {
	...
	componentWillMount(props) {
        this.props.navigation.setParams({
            title: this.props.activeName || '充值'
        });
	}

	componentDidMount() {
		// this.backHandler = BackHandler.addEventListener('hardwareBackPress', this.handleBackPress);
		this.props.activeCode == 'PPB' && this.setState({ visible: true }, () => console.log('visible', this.state.visible))
	}

	componentWillUnmount() {
		if(this.props.froms == 'promotions') {
			window.goHome && window.goHome()
			window.bonusState && Actions.PromotionsBank({BonusData: this.props.BonusData, data: this.props.data,froms: 'DepositCenter'})
		}
		if(this.props.activeCode === 'PPB') {
			Actions.PPBSecondStep({ secondStepData: this.props.checkcallbackDate.payCallback, activeDeposit: this.props.activeCode });
		}
		//虚拟币充值跳转
