...
class transfer extends React.Component {
	...
	Bonus(key) {
		fetchRequest(
			ApiPort.Bonus + "?transactionType=Transfer&wallet=SB&",
			"GET"
		)
			.then(data => {
	...
	okBonusId(value) {
		...
		fetchRequest(ApiPort.BonusApplications, "POST", data)
			.then(data => {
				if(data && data.bonusResult && data.bonusResult.message == 'Success') {
					//跳转其他页面
					Actions.pop()
								...
								{AboutPopup && 
									...
									<View style={styles.Popover}> 
										<TouchableOpacity onPress={() => { this.closePopver() }}  style={styles.PopoverConten}>  
											...
											<Text style={{ color: '#fff', paddingRight: 5, fontSize: 12 }}>包含沙巴体育, BTi体育, IM体育和电竞​</Text>
