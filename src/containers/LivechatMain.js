...
class LivechatMain extends React.Component {
    ...
    componentWillMount(props) {
       ...
       componentWillUnmount() {
         ...
         //拿客服链接
  getLiveChatX(betInfo = ''){
    fetchRequest(ApiPort.LiveChat, "GET")
      .then(data => {
          console.log(data,'111')
          console.log('newURL',data.url + betInfo);
          //LiveChatX = data.url;
            this.setState({
                LiveChatXT: data.url + betInfo,
                LiveChatXTSrc: data.url,
    ...
    render() {
        ...
        window.LiveChatOpenGlobe = (betInfo = '') => {
          console.log('betinfo',betInfo);
            ...
            if(LiveChatXT == ''){
                this.getLiveChatX(betInfo)
            } else {
              //更新鏈接
              const newUrl = this.state.LiveChatXTSrc + betInfo;
              if (newUrl != LiveChatXT) { //url有變化就重開
