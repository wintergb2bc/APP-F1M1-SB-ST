...
import { StyleSheet, Text, View, Modal, TouchableOpacity, Dimensions, TextInput, Image, Platform } from 'react-native'
import { Toast, DatePicker, List, Picker, WhiteSpace } from 'antd-mobile-rn'
...
class BettingHistory extends React.Component {
    ...
    sendEmailVerfication() {
        ...
        Toast.loading("发送中...", 100);
        fetchRequest(window.ApiPort.EmailVerify, "POST", data).then(res => {
            Toast.hide();
            // 发送异常
            if (!res.isSuccess) {
                let resendCounter = res.result.resendCounter
    ...
    verifyEmailTAC() {
        ...
        Toast.loading("验证中...", 100);
        fetchRequest(window.ApiPort.EmailTAC, "POST", data)
            .then(res => {
                Toast.hide();
                if (res) {
                    if (!res.isSuccess) {
    ...
    submitMemberInfor() {
        const { province, address, nameErr, profileMasterDataIndex, profileMasterData, memberInfo, name, question, secretQuestionsData, secretQuestionsIndex, countryData, countryIndex, birthdayDate, sexIndex } = this.state
        ...
        const params = {
            ...
            dob: memberInfo.DOB ? moment(memberInfo.DOB).format('YYYY-MM-DD') : birthdayDate,
        };
    ...
    render() {
        ...
        return <View style={[styles.viewContainer, { backgroundColor: '#fff' }]}>
            ...
            >
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 25 }}>
                    {
                        Withdrawalverification.map((v, i) => {
                            return <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                {
                                    WithdrawalverificationIndex > i && <View style={{
                                        width: 30,
                                        height: 30,
                                        borderRadius: 10000,
                                        alignItems: 'center',
                                ...
                                <View style={{
                                    height: 2,
                                    backgroundColor: WithdrawalverificationIndex - 1 < i ? '#D6D6D6' : '#00A6FF',
                                    width: 50,
                                    marginHorizontal: ((width - 20 - 4 * 30 - 50 * 3) / (3.2)) / 2
                                }}></View>
                            </View>
                        })
                    }
                ...
                {
                    WithdrawalverificationIndex == 0 && <View>
                        <View style={{
                            paddingBottom: 15,
                            borderBottomWidth: 1,
                            borderBottomColor: '#E3E3E8'
                        }}>
                            <Text style={{ fontWeight: 'bold', marginBottom: 10, fontSize: 16 }}>验证身份证号码</Text>
                            <Text style={{ color: '#999999', fontWeight: '400' }}>请输入您的有效身份证件以验证您的帐户</Text>
                        </View>

                        <View style={{ marginTop: 20 }}>
                            <Text style={{ color: '#999999', marginBottom: 10 }}>身份证号码 (18位）</Text>
                            <TextInput
                                value={IdentityCard}
                            ...
                            {
                                IdentityCard.length > 0 && IdentityCard.length < 18 && <TextInput
                                    value={'身份证号码格式错误'}
                                    onChangeText={IdentityCard => {
                                        let num = IdentityCard.replace(/[^0-9a-zA-Z]/g, '')
                                        this.setState({
                        ...
                        <TouchableOpacity
                            onPress={() => {
                                IdentityCard.length == 18 && this.submitMemberInforIdentityCard()

                                PiwikEvent('Verification', 'Next', 'NationalID_Verification_WithdrawalPage')
                            }}
                            style={{
                                height: 40,
                                alignItems: 'center',
                                ...
                                marginBottom: 15,
                                marginTop: 20,
                                backgroundColor: IdentityCard.length == 18 ? '#00A6FF' : '#EFEFF4'
                            }}>
                            <Text style={{
                ...
                {
                    WithdrawalverificationIndex == 1 && <View>
                        <View style={{
                            paddingBottom: 15,
                            borderBottomWidth: 1,
                            borderBottomColor: '#E3E3E8'
                        }}>
                            <Text style={{ fontWeight: 'bold', marginBottom: 10, fontSize: 16 }}>完善个人资料 </Text>
                            <Text style={{ color: '#999999', fontWeight: '400' }}>填写您的出生日期及联系地址</Text>
                        </View>

                        <View style={{ marginTop: 20 }}>
                            <Text style={{ color: '#999999', marginBottom: 10 }}>出生日期</Text>
                            {
                                <DatePicker
                                ...
                                >
                                    <List.Item styles={StyleSheet.create(ListItemstyles)}>
                                        <View style={[styles.limitListsInput, {
                                            ...
                                            {
                                                birthdayDate.length > 0 && <Text>{moment(new Date(birthdayDate)).format('YYYY年MM月DD日')}</Text>
                                            }

                                            {
                                                !isbirthdayDate && <Image
                                                    resizeMode='stretch'
                        ...
                        <View style={{ marginTop: 20 }}>
                            <Text style={{ color: '#999999', marginBottom: 10 }}>省市/自治市</Text>
                            <Picker
                                disabled={isprovince}
                                title='选择地区'
                                onChange={value => {
                        ...
                        <View style={{ marginTop: 20 }}>
                            <Text style={{ color: '#999999', marginBottom: 10 }}>联系地址</Text>
                            <TextInput
                                editable={!isAddress}
                                value={address}
                        ...
                        <TouchableOpacity
                            onPress={() => {
                                ...
                                PiwikEvent('Verification', 'Next', 'DOB_Verification_WithdrawalPage')
                            }}
                            style={{
                                height: 40,
                                alignItems: 'center',
                                ...
                                marginBottom: 15,
                                marginTop: 20,
                                backgroundColor: birthdayDate.length > 0 && province.length > 0 && address.length > 0 ? '#00A6FF' : '#EFEFF4'
                            }}>
                            <Text style={{
                                color: birthdayDate.length > 0 && province.length > 0 && address.length > 0 ? '#FFFFFF' : '#BCBEC3',
                                fontWeight: 'bold',
                                fontSize: 15
                            }}>提交</Text>
                        </TouchableOpacity>
                ...
                {
                    WithdrawalverificationIndex == 2 && verificaType.length > 0 && <View>
                        {
                            verification &&
                            <KeyboardAwareScrollView resetScrollToCoords={{ x: 0, y: 0 }}>
                                <View style={{ marginBottom: 20 }}>
                                    {/* <WhiteSpace size="lg" /> */}
                                    <View>
                                        <View style={{ backgroundColor: '#fff', borderRadius: 10, paddingBottom: 20 }}>
                                            ...
                                            <View style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row' }}>
                                                    ...
                                                    <View style={{ borderRadius: 10, backgroundColor: '#efeff4', paddingLeft: 15, width: (width - 60) / 1.2 }}>
                                                        <Text style={{ lineHeight: 45, color: '#000' }}>{maskPhone4(phone)}</Text>
                                                    </View>
                                                </View>
...

                                        {
                                            ...
                                            <View style={{ padding: 10, borderRadius: 10, paddingBottom: 20 }}>
                                                <View style={{ padding: 15, backgroundColor: '#F4F4F4', borderRadius: 10 }}>
                                                    ...
                                                    <Touch style={codeAgainPhone && issubmitBtn ? styles.addBtn : styles.addBtnAgain} onPress={() => { this.submitBtn() }}>
                ...
                {
                    WithdrawalverificationIndex == 3 && verificaType.length > 0 && <View>
                        {
                            verification &&
                            <KeyboardAwareScrollView resetScrollToCoords={{ x: 0, y: 0 }}>
                                <View style={{ marginBottom: 20 }}>
                                    ...
                                    <View>
                                        <View style={{ backgroundColor: '#fff', borderRadius: 10, paddingBottom: 20 }}>
                                            ...
                                            <View style={{ borderRadius: 10, backgroundColor: '#efeff4', paddingLeft: 15 }}>
                                                <Text style={{ lineHeight: 45, color: '#000' }}>{maskEmail(email)}</Text>
                                            </View>
                                            <View style={{ borderRadius: 10, backgroundColor: '#efeff4', marginTop: 15, paddingLeft: 15 }}>
                                                <Text style={{ color: '#999999', fontSize: 12, lineHeight: 45 }}>{verificationType[verificaType].txt3}<Text onPress={() => { LiveChatOpenGlobe() }} style={{ color: '#00a6ff', fontSize: 12 }}>在线客服</Text></Text>
                                            </View>
...

                                        {
                                            ...
                                            <View style={{ padding: 10, borderRadius: 10, paddingBottom: 20 }}>
                                                <View style={{ padding: 15, backgroundColor: '#F4F4F4', borderRadius: 10 }}>
                                                    ...
                                                    <View style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', marginTop: 10 }}>
                                                        <Touch style={codeAgainEmail && issubmitBtn ? styles.addBtn : styles.addBtnAgain} onPress={() => { this.submitBtn() }}>
                                                            <Text style={{ color: '#fff', textAlign: 'center', lineHeight: 45 }}>立即验证</Text>
                                                        </Touch>
