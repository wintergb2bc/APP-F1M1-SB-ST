...
let verificationType = {
  phone: {
    title: "手机接收验证码 ",
    txt1: "为了确保帐户安全，请在添加提款钱包之前验证您的手机号码。",
    txt2: "手机号码",
...
class SetVerification extends React.Component {
    ...
    this.props.navigation.setParams({
      ...
      rightButton: () => {
        return <View style={styles.gameButtonBox}>
          ...
          }} style={[styles.hiddenView]}></TouchableOpacity>
        </View>
      },
      rightButtonImage: () => {
        return null
    ...
    fetchRequest(window.ApiPort.SendSmsOTP + 'phoneNumber=' + number + '&', "POST")
      .then(res => {
        ...
        if (!res.isSuccess) {
          ...
          this.setState({
            verifyTimes: res.attempts,
            isSHowModal1: res.attempts <= 0
          })
        } else {
  ...
  VerifySmsOTP() { // 111
    ...
    fetchRequest(window.ApiPort.VerifySmsOTP + 'passCode=' + this.state.verificationCode + '&', "POST").then(data => {
      Toast.hide();
      if (data.isSuccess) {
        ...
        this.setState({
          ...
          errorMessageText1: data.message || "验证失败"
        })
        this.clearCode()
      }
    });
  ...
  addCryptoWallet() {
    ...
    fetchRequest(window.ApiPort.CryptoWallet + "?", "POST", requestData).then(data => {
      ...
      if (data.isSuccess) {
        ...
        let walletAddressErrorList = data.walletAddressErrorList
...
{
                              errorMessageText1.length > 0 && <View style={{ backgroundColor: '#FEE0E0',
                              paddingVertical: 10,flex: 1,
                               alignItems: 'center', justifyContent: 'center', paddingHorizontal: 15, marginTop: 10, borderRadius: 4 }}>
                          ...
                          <View style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', marginTop: 10 }}>
                            {
                              verificaType == 'phone' &&
                              <Touch style={codeAgainPhone && issubmitBtn ? styles.addBtn : styles.addBtnAgain} onPress={() => { this.VerifySmsOTP() }}>
...
{
                        errorMessageText2.length > 0 && ctcWalletName.length > 0 && <View style={{ backgroundColor: '#FEE0E0', height: 40, justifyContent: 'center', paddingLeft: 15, marginTop: 10, borderRadius: 4 }}>
                    ...
                    <View style={{ marginBottom: 20 }}>
                      ...
                      {
                        errorMessageText.length > 0 && ctcWalletAddress.length > 0 && <View style={{ backgroundColor: '#FEE0E0', height: 40, justifyContent: 'center', paddingLeft: 15, marginTop: 10, borderRadius: 4 }}>
                          <Text style={{ color: '#EB2121' }}>{errorMessageText}</Text>
                        </View>
                      }

                      {
                        !(ctcWalletAddress.length > 0 && WalletAddressRegex.test(ctcWalletAddress)) && ctcWalletAddress.length > 0 && <View style={{
