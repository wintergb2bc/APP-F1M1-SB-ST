...
class VendorIM extends VendorBase {
  ...
  _APIs = {
    ...
    //2.3 索取赛事和主要玩法资料
    getEventInfoMBT:
      (SportId = IMSports.SOCCER, Market = VendorMarkets.EARLY, BetTypeIds = [IMBetType.HANDICAP,IMBetType.OVERUNDER, IMBetType.CORRECTSCORE], MarketlineLevels = [1], EventGroupTypeIds = [1] ) => {
        if (Market === VendorMarkets.EARLY) { //早盤不主動查波膽，數據量太大
          BetTypeIds = [IMBetType.HANDICAP,IMBetType.OVERUNDER];
        }
        return this._imFetch('GETEVENTINFOMBT', {SportId, Market, BetTypeIds, MarketlineLevels, EventGroupTypeIds});
      },

    //2.4 索取 DELTA 赛事和主要玩法详情
    getDeltaEventInfoMBT:
      (SportId = IMSports.SOCCER, Market = VendorMarkets.EARLY, BetTypeIds = [IMBetType.HANDICAP,IMBetType.OVERUNDER, IMBetType.CORRECTSCORE], MarketlineLevels = [1], Delta =  'Y') => {
        if (Market === VendorMarkets.EARLY) { //早盤不主動查波膽，數據量太大
          BetTypeIds = [IMBetType.HANDICAP,IMBetType.OVERUNDER];
        }
        return this._imFetch('GETDELTAEVENTINFOMBT', {SportId, Market, BetTypeIds, MarketlineLevels, Delta})
      },
    ...
    getSelectedEventInfo: (SportId= IMSports.SOCCER, EventIds= [123,456], OddsType= IMOddsType.HK, IsCombo= false, IncludeGroupEvents= true, BetTypeIds = null, PeriodIds = null) => {
    ...
    for(let category in eventIdsBySportAndOutRight) {
      if (eventIdsBySportAndOutRight[category]) {
        ...
        if (eventIds.normal && eventIds.normal.length >0) {
          ...
          const normalPromise = this._APIs.getSelectedEventInfo(sportId, eventIds.normal, OddsType, false, true, BetTypeIds)
            .then(async jsonData => {
