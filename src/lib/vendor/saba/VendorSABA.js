...
import {vendorSettings, getTokenFromGatewayImpl} from '../vendorSettingSABA';
...
class VendorSABA extends VendorBase {
  ...
  async getEvents(SportId = SABASports.SOCCER, MarketId = VendorMarkets.EARLY, sortWay = SortWays.LeagueName, startDate = null, endDate = null
    ...
    if(MarketId !== VendorMarkets.FAVOURITE) {
      ...
      return Promise.all(queryPromises).then(async resultArray => {
        ...
        return new PollingResult(allEvents,allChanges)
      });
  ...
  getEventsPolling_GET(onUpdateCallback, SportId = SABASports.SOCCER, MarketId = VendorMarkets.EARLY, sortWay = SortWays.LeagueName, startDate = null, endDate = null, extraConfigs= {}, uniqueName = '') {
    ...
    const preCacheQuery = () => {
      ...
      if ([SABASports.SOCCER, SABASports.BASKETBALL].indexOf(SportId) === -1) {
        //只有足球和籃球支持緩存
        return new Promise(resolve => resolve(null));
      }
        ...
        .then(jsonData => {
          ...
          if (jsonData.price !== undefined && jsonData.price !== '') {
            ...
            copySelection.OddsList.map(item => {
              if (item.OddsType === queryOption.oddsType) {
          ...
          if (jsonData.status) {
            ...
            //TODO: saba處理 IsDecimalType 僅限歐洲盤
            if (jsonData.isDecimalType && queryOption.oddsType !== 3) {
          ...
          const selection = SelectionData.clone(copySelection); //利用clone重新配置
          ...
          .then(jsonData => {
            ...
            let selections = Selections.map(sourceSelectionData => {
              ...
              if (targetPriceInfos && targetPriceInfos.length > 0) {
                ...
                if (priceInfo.price !== undefined && priceInfo.price !== '') {
                  copySelection.Odds = priceInfo.price;
                  copySelection.DisplayOdds = priceInfo.price;
                  //更新oddsList
                  copySelection.OddsList.map(item => {
                    if (item.OddsType === queryOption.oddsType) {
                      item.OddsValues = priceInfo.price
                    }
                  })
                }
                if (priceInfo.status) {
      ...
      return this._SABABettingFetch(SABADataTypes.PLACEPARLAYBETS, queryOption)
        .then(jsonData => {
          ...
          return new BetResultData(
            ...
            isPending,
            pendingQueryId,
            jsonData.currentComboInfo ? jsonData.currentComboInfo[0].comboType : 0,
            0, //saba未提供，不填(實際也沒用到)
