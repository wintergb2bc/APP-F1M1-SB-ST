...
class Smartcoach extends Component {
	...
	initSmartCoach = async () =>{
		...
		if (CachedSCMatchId) {
			this.props.Disbandsmartcoach(false);
		...
		if (!token) {
			this.props.Disbandsmartcoach('沒有獲取到 sc token')
			return;
		}
		...
		if (scMatchId) {
			...
			this.props.Disbandsmartcoach('沒有獲取到 sc matchId')
		}
	}
	...
	*/
	...
	render() {
		...
		let IsNull3 = Home_Away == '' || !Home_Away || Home_Away.length <=0;

		console.log('IsNull0IsNull0',IsNull0,IsNull1,IsNull2,IsNull3,)
			...
			<View style={{flex: 1, backgroundColor: '#EFEFF4'}}>
				...
				<View style={styles.Tab}>
					{tabList.map((data, key) => {
						...
						let disabled1 = IsNull0 && key == 0
						/* 只显示有数据的菜单 反之Disabled置灰按钮并隐藏内容 */
						if (!disabled && !HaveData && !this.state.HaveData) {
							this.state.Tabactive = key;
							HaveData = true;
						}
						return (
							<Touch
								key={key}
								onPress={() => {
									if(IsNull0 && key == 0) { return }
									if(IsNull1 && key == 1) { return }
									if(IsNull2 && key == 2) { return }
									if(IsNull3 && key == 3) { return }
				...
				{!HaveData && !this.state.HaveData && <Skeleton />}
			</View>
		);
	}
}
