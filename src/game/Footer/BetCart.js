...
class BetCart extends React.Component {
	...
	ClickTabmenu = (type) => {
		if(type == 1) {
			PiwikEvent('BetCart', 'Click', 'Singlebet')
		} else if(type == 1) {
			PiwikEvent('BetCart', 'Click', 'Combobet')
		} else {
			PiwikEvent('BetCart', 'Click', 'SystemCombo')
		}
								...
								<View style={StartBettingloading == 1 ? styles.BottomBtnLoding : styles.BottomBtn}>
									...
									<Touch
										...
										onPress={() => {
											...
											this.props.betCart_setIsComboBet(false, Vendor.configs.VendorName);
											this.props.CloseBottomSheet();
											PiwikEvent('BetCart', 'Click', 'Betcart_Continue')
										}}
									>
									...
									<Touch
										...
										onPress={() => {
											...
											if(this.props.EuroCupBet || this.props.EuroCupBetDetail) {
												PiwikEvent('Engagement Event', 'View', 'Record_BetSlip_EUROPage')
												window.ChangeTabs && window.ChangeTabs(2)
											} else {
												PiwikEvent('BetCart', 'Close', 'Back_Mainpage')
												Actions.betRecord()
											}
										}}
									>
